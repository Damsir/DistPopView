...
@interface DistPopView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<DistPopAction *> *actions;
...
@property (nonatomic, copy) DistPopActionBlock popActionBlock;

@end

@implementation DistPopView
...
- (void)setStyle:(DistPopViewStyle)style {
    ...
    _tableView.separatorColor = [DistPopViewCell bottomLineColorForStyle:_style];
...
- (void)initialize {
    ...
    _actions = @[];
    _isUpward = YES;
    _style = DistPopViewStyleDefault;
    _arrowStyle = DistPopViewArrowStyleTriangle;
    ...
    [_tableView registerClass:[DistPopViewCell class] forCellReuseIdentifier:kPopoverCellReuseId];
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    // 如果是菱角箭头的话, 箭头宽度需要小点.
    if (_arrowStyle == DistPopViewArrowStyleTriangle) {
        arrowWidth = 22.0;
    }
...
- (CGFloat)calculateMaxWidth {
    CGFloat maxWidth = 0.f, titleLeftEdge = 0.f, imageWidth = 0.f, imageMaxHeight = kPopoverViewCellHeight - PopViewCellVerticalMargin*2;
    CGSize imageSize = CGSizeZero;
    UIFont *titleFont = [DistPopViewCell titleFont];
    
    for (DistPopAction *action in _actions) {
        ...
        if (action.image) { // 存在图片则根据图片size和图片最大高度来重新计算图片宽度
            ...
            titleLeftEdge = PopViewCellTitleLeftEdge; // 有图片时标题才有左边的边距
...
#pragma GCC diagnostic pop
        ...
        CGFloat contentWidth = PopViewCellHorizontalMargin*2 + imageWidth + titleLeftEdge + titleWidth;
...
- (void)showToView:(UIView *)pointView actions:(NSArray<DistPopAction *> *)actions handler:(DistPopActionBlock)popActionBlock {
    ...
    _popActionBlock = popActionBlock;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DistPopViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellReuseId];
    cell.style = _style;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.12f animations:^{
        ...
        DistPopAction *action = _actions[indexPath.row];
        _actions = nil;
        if (self.popActionBlock){
            self.popActionBlock(action);
        }
        [_shadeView removeFromSuperview];
        [self removeFromSuperview];
    }];
